...
std::vector<Tensor> _prod_bw(
    ...
    if (prod_result.get_legacy_shape().without_padding() != grad.get_legacy_shape()) {
        if (dim == 3 || dim == -1) {
            ...
            Tensor new_slice_tensor = ttnn::slice(0, required, start_index, end_index, std::nullopt, std::nullopt, std::nullopt);
    ...
    if (dim == 3 || dim == -1) {
        ...
        if (reciprocal_input.get_legacy_shape()[1] % 32 != 0) {
            ...
            grad_result = ttnn::slice(0, result, start_index, end_index, std::nullopt, std::nullopt, std::nullopt);
        }
        grad_tensor.emplace_back(grad_result);
        return grad_tensor;
    }
