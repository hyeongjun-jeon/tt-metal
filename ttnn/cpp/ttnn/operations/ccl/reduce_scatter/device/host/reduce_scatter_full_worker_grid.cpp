...
static void add_worker_config_to_edm_builders(
    ...
    for (std::size_t c = 0; c < num_channels_per_edm; ++c) {
        ...
        if (sender_enabled) {
            bool choose_clockwise_edm_builder = is_in_clockwise_direction;// || is_linear);
        ...
        if (receiver_enabled) {
            bool choose_counter_clockwise_edm_builder = is_in_clockwise_direction;//(
                // (!is_linear && is_in_clockwise_direction) ||
                // (is_linear && ));
...
operation::ProgramWithCallbacks reduce_scatter_with_workers(
   ...
   ttnn::ccl::Topology topology) {
    ...
    // if (ring_index != 0) {
        ...
        // std::this_thread::sleep_for(std::chrono::seconds(5 * ring_index));
    // }
