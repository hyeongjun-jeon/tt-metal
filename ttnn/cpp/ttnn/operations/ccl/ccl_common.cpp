...
std::vector<TensorSlice> generate_slice_sequence_on_dim(
    ...
    auto generate_slice = [incr, &slices, &tensor_shape, &slice_shape, &worker_slice_shape, &tensor_slice_offset, &worker_slice_start_offset, fracture_dim, dim_start_offset, slice_size_on_dim](std::size_t i){
        if (fracture_dim == 0) {
            tensor_slice_offset.y += slice_size_on_dim * incr;
        } else {
            tensor_slice_offset.x += slice_size_on_dim * incr;;
        }
    ...
    for (int i = start_slice_index; i + incr != end_slice_index_exclusive; i += incr) {
        generate_slice(i);
    }
