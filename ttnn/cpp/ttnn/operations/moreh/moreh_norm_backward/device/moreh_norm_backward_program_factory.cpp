...
tt::tt_metal::Shape get_output_grad_shape(const Tensor &output_grad, const Tensor &input_grad, const std::vector<int64_t> &dims, const bool &keep_batch_dim) {
    if (keep_batch_dim)
    ...
    return tt::tt_metal::Shape(shape, padding);
}
...
MorehNormBackwardOperation::ProgramFactory::cached_program_t MorehNormBackwardOperation::ProgramFactory::create(
    ...
    tt::tt_metal::Shape output_grad_shape = get_output_grad_shape(output_grad, input_grad, operation_attributes.dims, operation_attributes.keepdim);
    ...
    auto [math_fidelity, math_approx_mode, fp32_dest_acc_en, packer_l1_acc] = get_compute_kernel_config_args(output_grad.device()->arch(), operation_attributes.compute_kernel_config);
    ...
    { static_cast<uint32_t>(tt::operations::primary::is_dram(input)), static_cast<uint32_t>(tt::operations::primary::is_dram(output)), static_cast<uint32_t>(tt::operations::primary::is_dram(output_grad)), static_cast<uint32_t>(input_grad_rank) };
    std::vector<uint32_t> writer_compile_time_args =
    { static_cast<uint32_t>(tt::operations::primary::is_dram(input_grad)) };
    const auto reader_kernels_id = tt::operations::primary::CreateReadKernel(program, reader_kernel_file, all_cores, reader_compile_time_args);
    const auto writer_kernels_id = tt::operations::primary::CreateWriteKernel(program, writer_kernel_file, all_cores, writer_compile_time_args);
    ...
    const auto compute_kernels_id_1 =
        tt::operations::primary::CreateComputeKernel(program, compute_kernel_file, {core_group_1, num_cols_per_core_group_1, compute_args_group_1},
        compute_defines,
        math_fidelity,
        fp32_dest_acc_en,
        math_approx_mode);
    ...
    if (!core_group_2.ranges().empty()) {
        ...
        compute_kernels_id_2 =
            tt::operations::primary::CreateComputeKernel(program, compute_kernel_file, {core_group_2, num_cols_per_core_group_2, compute_args_group_2},
        compute_defines,
        math_fidelity,
        fp32_dest_acc_en,
        math_approx_mode);
