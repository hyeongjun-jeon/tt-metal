...
ttnn::Tensor SliceOperation::invoke(
    ...
    TT_FATAL(input_rank == begins.size(), "Start and end must have the same size");
    TT_FATAL(begins.size() == ends.size(), "Start and end must have the same size");
    TT_FATAL(step.size() == begins.size(), "Step must have the same size as start and end");
    ...
    auto output_dim_i = [&modified_begins, &modified_step] (size_t i, std::array<uint32_t, 4> modified_ends) {
    ...
    for (int i = 0; i < input_rank; ++i) {
        ...
        TT_FATAL(modified_ends[i + rank_diff] >= modified_begins[i + rank_diff], "End must be greater than or equal to start");
    ...
    if (input_tensor.storage_type() != StorageType::DEVICE) {
        ...
        if (input_tensor.is_sharded() && input_tensor.memory_config() == memory_config &&
            ...
            in_place_unpad &= modified_begins[i] == 0 &&
                              tt::div_up(modified_ends[i], input_tensor.shard_spec().value().shape[0]) ==
            ...
            if (in_place_unpad) {
                ...
                std::size_t unpad_val = input_tensor_shape[-2] - modified_ends[-2];
