...
static std::pair<std::vector<DataFormat>, std::vector<DataFormat>> generate_unpack_data_formats(
    tt_hlk_desc& desc, DataFormat unpack_conditional_dst_format, bool fp32_dest_acc_en, bool preserve_fp32_precision) {
...
static void generate_data_format_descriptors(JitBuildOptions& options, const tt::ARCH arch) {
    ...
    if ((tt::is_all_fp32_formats(desc.input_buf_dataformat_arr) || options.preserve_fp32_precision) &&
    ...
    tie(unpack_src_formats_all_cbs, unpack_dst_formats_all_cbs) = generate_unpack_data_formats(
        desc, unpack_conditional_dst_format, options.fp32_dest_acc_en, options.preserve_fp32_precision);
