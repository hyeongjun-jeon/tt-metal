...
std::vector<chip_id_t> SystemMesh::get_mapped_physical_device_ids(const std::size_t requested_rows, const std::size_t requested_cols, const std::size_t row_offset, const std::size_t col_offset) const {
...
std::vector<Device*> SystemMesh::map_mesh_device(
    ...
    auto physical_device_ids = user_provided_physical_device_ids.empty() ?
        this->get_mapped_physical_device_ids(requested_num_rows, requested_num_cols, row_offset, col_offset) :
...
std::vector<int> get_t3k_physical_device_ids_ring() {
    ...
    auto [requested_num_rows, requested_num_cols] = instance.get_shape();
    auto physical_device_ids = instance.get_mapped_physical_device_ids(requested_num_rows, requested_num_cols);
    return physical_device_ids;
}
