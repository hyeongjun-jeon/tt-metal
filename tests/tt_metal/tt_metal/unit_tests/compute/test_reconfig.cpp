...
bool single_core_reconfig(tt_metal::Device* device, const ReconfigConfig& test_config) {
    ...
    auto compute_kernel = tt_metal::CreateKernel(
        ...
        tt_metal::ComputeConfig{
            .fp32_dest_acc_en = test_config.fp32_dest_acc_en ? true : false,
    ...
    std::vector<uint32_t> src0_vec = create_random_vector_of_bfp8(
    ...
    std::cout << "src0_vec[" << std::dec << 0 << "] = " << std::hex << src0_vec[0] << std::endl;
    std::cout << "src0_vec[" << std::dec << 100 << "] = " << std::hex << src0_vec[100] << std::endl;
    std::cout << "src1_vec[" << std::dec << 0 << "] = " << std::hex << src1_vec[0] << std::endl;
    std::cout << "src1_vec[" << std::dec << 1 << "] = " << std::hex << src1_vec[1] << std::endl;
    std::cout << "src2_vec[" << std::dec << 0 << "] = " << std::hex << src2_vec[0] << std::endl;
    std::cout << "src2_vec[" << std::dec << 1 << "] = " << std::hex << src2_vec[1] << std::endl;
    ////////////////////////////////////////////////////////////////////////////
    //                      Golden Generation
    ////////////////////////////////////////////////////////////////////////////
    ...
    // It's tricky to make a variable-type vector, so we're
    ...
    for (auto i = 0; i < temp_golden.size(); i++) {
        ...
        if (i < 2) {
            std::cout << "temp_golden[" << std::dec << i << "] = " << std::hex << temp_golden[i] << std::endl;
            std::cout << "golden0_fp32[" << std::dec << i << "] = " << std::hex << golden0_fp32[i] << std::endl;
            std::cout << "golden0_bfp16[" << std::dec << i << "] = " << std::hex << golden0_bfp16[i] << std::endl;
        }
    }
    if (!test_config.fp32_dest_acc_en) {
    ...
    std::cout << "dest0_buffer_data[" << std::dec << 0 << "] = " << std::hex << dest0_buffer_data[0] << std::endl;
    std::cout << "dest0_buffer_data[" << std::dec << 1 << "] = " << std::hex << dest0_buffer_data[1] << std::endl;
    std::cout << "dest1_buffer_data[" << std::dec << 0 << "] = " << std::hex << dest1_buffer_data[0] << std::endl;
    std::cout << "dest1_buffer_data[" << std::dec << 100 << "] = " << std::hex << dest1_buffer_data[100] << std::endl;
    std::cout << "packed_golden0[" << std::dec << 0 << "] = " << std::hex << packed_golden0[0] << std::endl;
    std::cout << "packed_golden0[" << std::dec << 1 << "] = " << std::hex << packed_golden0[1] << std::endl;
    std::cout << "packed_golden1[" << std::dec << 0 << "] = " << std::hex << packed_golden1[0] << std::endl;
    std::cout << "packed_golden1[" << std::dec << 100 << "] = " << std::hex << packed_golden1[100] << std::endl;
...
TEST_F(DeviceFixture, TileCopyReconfigExplicitSplitDstAcc) {
    ...
    //             log_info(LogTest, "Flag explicit_reconfig is {}", explicit_reconfig ? "true." : "false.");
    //             log_info(LogTest, "Flag split_src_reconfig is {}", split_src_reconfig ? "true." : "false.");
    //             log_info(LogTest, "Flag fp32_dest_acc_en is {}", fp32_dest_acc_en ? "true." : "false.");
